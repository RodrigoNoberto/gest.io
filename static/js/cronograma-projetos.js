...
(function(){
  ...
  /* tira acento decompondo em NFD e removendo os diacríticos combinantes.
     ...
     mesmo inline pros rótulos da planilha EAP.) */
  ...
  /* a coluna "Resp." da planilha EAP é texto livre e aceita "Rodrigo/Bruno".
  ...
  /* Marcas de período do Realizado: {'YYYY-MM-DD': marca}. Sanitiza chave
     ...
     com o mesmo conteúdo acusariam alteração onde não houve nenhuma. */
      ...
      etapas: Array.isArray(p.etapas) ? p.etapas.map(function(e){
        return {
          ...
          tarefas: Array.isArray(e.tarefas) ? e.tarefas.map(function(t){
            /* `realizado` sai do literal porque o status é derivado dele. A
               ordem das CHAVES do objeto continua idêntica — é ela que o
               projKey() serializa pra comparar com o BASE; só a ordem de
               cálculo mudou. */
            ...
            return {
              ...
              realizado: realizado,
              /* Não existe mais um `fca` por TAREFA (ago/2026). Ele foi
                 substituído pelo mapa por período: um FCA por tarefa não tinha
  ...
  function projKey(p){ return contentKey(normalizeProjeto(p)); }

  /* Object.create(null) pela mesma razão do byKey em computeAlocacao: a chave
     é um id vindo de JSON de fora, e um id "__proto__" gravaria no protótipo
     em vez de criar entrada. safeId() já barra isso, mas o custo aqui é zero. */
  function snapshotBase(data){
    var m = Object.create(null);
    ((data && data.projetos) || []).forEach(function(p){ m[p.id] = projKey(p); });
    return m;
  }
  ...
  */
  function recalcPendencias(){
    ...
    ((DATA && DATA.projetos) || []).forEach(function(p){
      vistos[p.id] = true;
      if(BASE[p.id] === undefined || projKey(p) !== BASE[p.id]) salvar.push(p);
    });
    Object.keys(BASE).forEach(function(id){
      if(!vistos[id]) excluir.push(id);
    });
  ...
  function renderSaveUI(){
    ...
    if(el){
      ...
      var n = nPend();
      el.textContent = (SAVE_MODE === 'dirty')
        ? (n + (n === 1 ? ' projeto não salvo' : ' projetos não salvos'))
        : s.t;
      el.className = 'cr-saveind ' + s.c;
    }
  ...
  /* Os saves são serializados numa corrente: dois cliques rápidos disputariam
     o mesmo CAS e o perdedor gastaria uma tentativa de retry à toa. Cada elo
     lê o DATA do momento em que roda, então nada se perde na fila. O segundo
  ...
  /* Ctrl+S / Cmd+S — o gesto que todo mundo já tem no dedo pra "encerrei, grava".
     ...
     o "salvar página como", que não é o que ninguém quis. */
  ...
  /* ÚNICO vocabulário de estado da página. É o status da tarefa, declarado
     período a período na linha R — o equivalente às letras que a planilha EAP
     legada tinha digitadas ali (D, A, R, C). A lista NÃO inclui "em branco":
  ...
  /* ---------- COMENTÁRIOS (leitura) ----------
     ...
     Não altera nada, então não chama markDirty() nem render(). */
  ...
  /* ---------- CLIQUE NUMA CÉLULA DA LINHA "R" ----------
     ...
     recente (statusDeMarcas). Até ago/2026 este menu tinha uma segunda seção
  ...
  /* ---------- EXPORT / IMPORT (um projeto por arquivo) ----------
     UM formato só:

       {version:1, scope:'projeto', projetos:[p]}

     Até ago/2026 existia também o arquivo de página inteira
     ({version:1, projetos:[...]}, sem `scope`), com "Exportar tudo" e um import
     que tratava o arquivo como a verdade da página: projeto que não estivesse
     nele era REMOVIDO. Era o ciclo clássico de perda de dado — exportar hoje,
     alguém criar/editar outro projeto amanhã, reimportar o arquivo de hoje e
     apagar o trabalho dessa pessoa sem ninguém perceber. Os dois lados saíram
     (o botão e a aceitação do formato no import): um arquivo de um projeto só
     nunca carrega opinião sobre os projetos que ele não contém.

     Backup da página inteira não é mais responsabilidade desta tela — o banco é
     a fonte, e o backup do MySQL é administrado pela TI. */
  ...
  /* Decide o que o projeto do arquivo vai fazer com a tela ANTES de gravar
     ...
     recalcPendencias() ver uma exclusão seguida de uma criação. */
  ...
  function aplicarImportJSON(plano, body){
    ...
    DATA = normalizeData(DATA);  // sanitização final, igual ao import de planilha
  ...
  /* ---------- IMPORT DE PLANILHA EAP (.xlsx) ----------
     Esta página nasceu de uma planilha Excel legada — a "EAP — Estrutura
     Analítica do Projeto", com uma aba por projeto. Este bloco lê essa
     planilha e converte pro formato desta página, pra não ter que redigitar
     cronograma que já existe.

     O layout é descoberto por RÓTULO, não por endereço fixo de célula, pra
     sobreviver a variações entre planilhas:

       • cabeçalho — "Projeto :", "Líder :", "Data da Revisão >>". O valor é a
         primeira célula não vazia à direita do rótulo, na mesma linha;
       • a célula "ETAPAS / AÇÕES" ancora a tabela: define a coluna do número
         WBS e, na linha seguinte, estão "Início", "Fim", "Conclusão",
         "Novo Prazo" e "P x R". A linha do tempo começa depois de "P x R";
       • cada tarefa ocupa DUAS linhas (mescladas na vertical na planilha): a
         linha "P" (planejado) e a linha "R" (realizado). O número WBS separa
         os níveis — inteiro ("2") = etapa, decimal ("2.1") = tarefa;
       • o status NÃO vive numa coluna: são as letras da legenda digitadas nas
         células da linha do tempo da linha "R" (C, CC, D, A, R, N).

     A planilha não tem FCA nem Comentários — esses campos entram vazios e são
     preenchidos aqui no Portal (o FCA pela célula do período reprogramado, ver
     crFcaPeriodo). Importar uma planilha que traz um período marcado "R" cria,
     portanto, uma pendência de FCA pra cada um deles — o que é o comportamento
     certo: a reprogramação veio declarada, o motivo não. */

  var XLSX_LOCAL_SRC = '/static/js/xlsx.full.min.js';
  var XLSX_MAX_BYTES = 15 * 1024 * 1024;
  var EAP_ANCHOR = 'etapas / acoes';
  var EAP_MARCAS_ANDAMENTO = ['D','A','R','N']; // Em dia, Atrasada, Reprogramar, Novo Prazo

  /* Cópia local (~950 KB), servida pelo próprio Portal — sem CDN externo.
     Carregada sob demanda: quem só está olhando o cronograma, ou está em modo
     apresentação, nunca baixa a lib. */
  function loadSheetJS(){
    return new Promise(function(resolve, reject){
      if(window.XLSX) return resolve(window.XLSX);
      var s = document.createElement('script');
      s.src = XLSX_LOCAL_SRC;
      s.onload = function(){
        if(window.XLSX) resolve(window.XLSX);
        else reject(new Error('a biblioteca carregou sem expor XLSX'));
      };
      s.onerror = function(){ reject(new Error('não foi possível carregar '+XLSX_LOCAL_SRC)); };
  ...
  /* rótulo comparável: minúsculo, sem acento, sem a pontuação decorativa dos
     rótulos da planilha ("Projeto : " → "projeto", "Data da Revisão >>" →
     "data da revisao") */
  ...
  /* Leitura por endereço A1 em vez de XLSX.utils.sheet_to_json: além de dar
     acesso ao .f (preciso pra saber se a coluna Conclusão é fórmula), evita
     montar objeto com chaves tiradas do arquivo. */
  function colLetter(c){
    var s = '', n = c+1;
    while(n > 0){ var m = (n-1)%26; s = String.fromCharCode(65+m)+s; n = Math.floor((n-1)/26); }
    return s;
  }
  function cellAt(ws, r, c){ return (r<0 || c<0) ? null : (ws[colLetter(c)+(r+1)] || null); }
  function cellText(ws, r, c){ var x=cellAt(ws,r,c); return (x && x.v!=null) ? String(x.v).trim() : ''; }
  function cellNum(ws, r, c){ var x=cellAt(ws,r,c); return (x && typeof x.v==='number') ? x.v : null; }
  function cellHasFormula(ws, r, c){ var x=cellAt(ws,r,c); return !!(x && x.f); }

  /* serial de data do Excel → 'YYYY-MM-DD'. Só aritmética UTC de propósito:
     com cellDates:true o SheetJS devolve Date montado em UTC e a leitura em
     fuso negativo (Brasil) volta um dia — o mesmo bug que parseISO() evita. */
  function serialToISO(n){
    if(typeof n !== 'number' || !isFinite(n)) return '';
    n = Math.floor(n);
    // <61 cai no bug histórico do 1900-02-29 do Excel (e descarta, de graça,
    // célula numérica que não é data); >2958465 passa de 9999-12-31.
    if(n < 61 || n > 2958465) return '';
    var d = new Date((n - 25569) * 86400000); // 25569 = serial de 1970-01-01
    return d.getUTCFullYear() + '-' +
           String(d.getUTCMonth()+1).padStart(2,'0') + '-' +
           String(d.getUTCDate()).padStart(2,'0');
  }

  /* Letras da legenda digitadas na linha "R" (realizado) — a linha logo abaixo
     da linha "P" da tarefa. Só lê as colunas da linha do tempo, nunca a coluna
     "P x R" (cujo conteúdo é sempre a letra "R" e não é status).
     Devolve as letras únicas (pro status da tarefa) E em qual coluna cada uma
     estava: desde ago/2026 a página guarda o Realizado período a período, então
     a posição da letra deixou de ser descartável — é ela que diz QUAL período
     a planilha marcou. */
  function marcasLinhaR(ws, rP, colPR, tStart, C1){
    var rR = rP + 1;
    if(colPR >= 0 && normLabel(cellText(ws, rR, colPR)) !== 'r') return { letras:[], cols:[] };
    var letras = [], cols = [];
    for(var c=tStart; c<=C1; c++){
      var tx = cellText(ws, rR, c).toUpperCase();
      if(!tx) continue;
      if(letras.indexOf(tx) < 0) letras.push(tx);
      cols.push({ c:c, tx:tx });
    }
    return { letras:letras, cols:cols };
  }
  /* Legenda da planilha → marca de período desta página. "CC" (cancelado /
     stand-by) fica de fora: não tem cor equivalente aqui, e inventar uma
     pintaria de concluído/atrasada uma tarefa que na verdade foi suspensa. */
  var EAP_MARCA_POR_LETRA = { C:'concluida', D:'andamento', A:'atrasada', R:'reprogramada', N:'reprogramada' };
  /* Legenda da planilha → os 3 valores internos de t.status. Fallback: só
     decide quando a linha R não deixou marca nenhuma (planilha sem letras, ou
     só com "CC"); havendo marca, quem manda é statusDeMarcas. "Atrasada" não
     entra aqui porque não é valor de t.status — é cor derivada da Data de
     Revisão (ver computeProjectView). */
  function statusEap(marcas){
    /* Comparação por valor EXATO: "CC" (cancelado) contém a letra "C"
       (concluído), então testar por substring transformaria toda tarefa
       cancelada em concluída. */
    if(marcas.indexOf('C') >= 0) return { status:'concluida', cancelada:false };
    if(marcas.indexOf('CC') >= 0) return { status:'nao_iniciada', cancelada:true };
    for(var i=0;i<marcas.length;i++){
      if(EAP_MARCAS_ANDAMENTO.indexOf(marcas[i]) >= 0) return { status:'em_andamento', cancelada:false };
    }
    return { status:'nao_iniciada', cancelada:false };
  }
  function chaveDoRotulo(lb){
    if(lb === 'projeto') return 'nome';
    if(lb === 'lider') return 'lider';
    if(lb === 'data da revisao') return 'dataRevisao';
    return '';
  }

  /* Converte uma aba em {aba, projeto, avisos, stats} — ou null se a aba não é
     uma EAP (sem a célula-âncora, sem coluna Fim ou sem nenhuma etapa). */
  function parseEapSheet(ws, aba){
    if(!ws || !ws['!ref']) return null;
    var rg = window.XLSX.utils.decode_range(ws['!ref']);
    // tetos de sanidade: uma EAP real é pequena; evita varredura absurda numa
    // planilha com range inflado.
    var R0 = rg.s.r, R1 = Math.min(rg.e.r, 5000), C0 = rg.s.c, C1 = Math.min(rg.e.c, 200);

    /* 1) âncora "ETAPAS / AÇÕES" */
    var aR = -1, aC = -1;
    for(var r=R0; r<=R1 && aR<0; r++){
      for(var c=C0; c<=C1; c++){
        if(normLabel(cellText(ws,r,c)) === EAP_ANCHOR){ aR=r; aC=c; break; }
      }
    }
    if(aR < 0) return null;

    /* 2) colunas, pelos rótulos da linha da âncora e da linha seguinte */
    var hR = aR + 1;
    var col = { resp:-1, inicio:-1, fim:-1, conclusao:-1, novoPrazo:-1, pr:-1 };
    for(var ci=aC; ci<=C1; ci++){
      var la = normLabel(cellText(ws, aR, ci));
      if(col.resp<0 && (la==='resp.' || la==='resp' || la==='responsavel')) col.resp = ci;
      var lh = normLabel(cellText(ws, hR, ci));
      if(col.inicio<0 && lh==='inicio') col.inicio = ci;
      else if(col.fim<0 && lh==='fim') col.fim = ci;
      else if(col.conclusao<0 && lh==='conclusao') col.conclusao = ci;
      else if(col.novoPrazo<0 && lh==='novo prazo') col.novoPrazo = ci;
      else if(col.pr<0 && lh==='p x r') col.pr = ci;
    }
    if(col.fim < 0) return null; // sem coluna Fim não há cronograma a montar

    /* 3) linha do tempo: começa depois da última coluna conhecida. A unidade
          sai da distância entre os dois primeiros buckets do cabeçalho. */
    var tStart = Math.max(aC, col.resp, col.inicio, col.fim, col.conclusao, col.novoPrazo, col.pr) + 1;
    var serials = [];
    for(var ct=tStart; ct<=C1 && serials.length<2; ct++){
      var sv = cellNum(ws, hR, ct);
      if(sv && sv >= 61) serials.push(sv);
    }
    var unidade = 'meses';
    if(serials.length === 2 && Math.abs(serials[1]-serials[0]) < 20) unidade = 'semanas';

    /* coluna da linha do tempo → data de início do período correspondente, na
       granularidade detectada. É o que permite importar as letras da linha "R"
       período a período (o formato que a página guarda em tarefa.realizado).
       Coluna cujo cabeçalho não é uma data fica fora do mapa e a letra dela é
       ignorada — melhor perder uma marca que carimbar o período errado. */
    var colPeriodoISO = {};
    for(var cp=tStart; cp<=C1; cp++){
      var dh = parseISO(serialToISO(cellNum(ws, hR, cp)));
      if(dh) colPeriodoISO[cp] = periodoISO(bucketKey(dh, unidade), unidade);
    }

    /* 4) cabeçalho do projeto */
    var head = { nome:'', lider:'', dataRevisao:'' };
    for(var rh=R0; rh<aR; rh++){
      for(var ch=C0; ch<=C1; ch++){
        var key = chaveDoRotulo(normLabel(cellText(ws, rh, ch)));
        if(!key || head[key]) continue;
        for(var k=ch+1; k<=C1; k++){
          var cel = cellAt(ws, rh, k);
          if(!cel || cel.v==null || String(cel.v).trim()==='') continue;
          head[key] = (key==='dataRevisao') ? serialToISO(cel.v) : String(cel.v).trim();
          break;
        }
      }
    }
    ...
    /* 5) corpo da tabela */
    var avisos = [], etapas = [], cur = null, semEtapa = null;
    var stats = { total:0, concluida:0, em_andamento:0, nao_iniciada:0, canceladas:0, marcas:0 };
    var tituloEnd = (col.resp > aC) ? col.resp-1 : aC+2;

    for(var rb=hR+1; rb<=R1; rb++){
      var wbs = cellText(ws, rb, aC);
      if(!wbs) continue;
      var ehEtapa = /^\d+$/.test(wbs);
      var ehTarefa = /^\d+(?:\.\d+)+$/.test(wbs);
      // qualquer outra coisa (">> CRONOGRAMA DO PROJETO", bloco STATUS do
      // rodapé, linhas em branco) é ignorada — não precisa de regra de parada.
      if(!ehEtapa && !ehTarefa) continue;

      var titulo = '';
      for(var cb=aC+1; cb<=tituloEnd; cb++){
        var tx = cellText(ws, rb, cb);
        if(tx){ titulo = tx; break; }
      }
      if(!titulo) continue; // número sem descrição = resíduo de fórmula

      if(ehEtapa){
        cur = { id:uid(), titulo:titulo, tarefas:[] };
        etapas.push(cur);
        continue;
      }
      ...
      if(!alvo){
        if(!semEtapa){
          semEtapa = { id:uid(), titulo:'(sem etapa)', tarefas:[] };
          etapas.push(semEtapa);
          avisos.push('Tarefas que aparecem antes da primeira etapa foram agrupadas em "(sem etapa)".');
        }
        alvo = semEtapa;
      }

      var fim = serialToISO(cellNum(ws, rb, col.fim));
      var inicio = serialToISO(cellNum(ws, rb, col.inicio));
      if(!fim){
        avisos.push('"'+wbs+' '+titulo+'": sem data de Fim na planilha — importada, mas a página vai marcá-la com ⚠ até você completar.');
      }

      var marcasR = marcasLinhaR(ws, rb, col.pr, tStart, C1);
      var st = statusEap(marcasR.letras);
      if(st.cancelada) stats.canceladas++;

      /* linha "R" da planilha → marcas de período. Duas letras diferentes na
         mesma semana/mês (acontece em visão mensal, com uma letra por semana)
         colapsam na pior, mesma regra da tela. */
      var realizado = {};
      marcasR.cols.forEach(function(cel){
        var isoP = colPeriodoISO[cel.c], marca = EAP_MARCA_POR_LETRA[cel.tx];
        if(isoP && marca) realizado[isoP] = piorCor(realizado[isoP], marca);
      });
      stats.marcas += Object.keys(realizado).length;
      /* Status derivado das marcas, igual à tela: a letra mais à direita manda.
         O statusEap só decide quando não sobrou marca nenhuma — planilha sem
         letras na linha R, ou só com "CC" (cancelada), que não vira marca.
         Sem isto, uma tarefa com "C" numa semana antiga e "D" na última seria
         importada como concluída (statusEap procura o C em qualquer posição):
         entraria no KPI de Conclusão e sumiria da Ocupação com "esconder
         concluídas", enquanto a linha R mostra "em dia" na última semana — KPI
         e tela discordando de origem. */
      var statusFinal = statusDeMarcas({ realizado:realizado }) || st.status;

      /* Novo Prazo só entra se for posterior ao Fim — mesma regra de
         salvarTarefa(). Importar valor que a própria página recusaria deixaria
         a tarefa num estado impossível de salvar depois. */
      var novoPrazo = serialToISO(cellNum(ws, rb, col.novoPrazo));
      if(novoPrazo && fim && novoPrazo <= fim){
        avisos.push('"'+wbs+'": Novo Prazo ('+fmtDateBR(novoPrazo)+') não é posterior ao Fim ('+fmtDateBR(fim)+') — descartado.');
        novoPrazo = '';
      }

      /* A coluna "Conclusão" da planilha normalmente é FÓRMULA — o 1º dia do
         mês do Fim planejado, que é exatamente o que esta página recalcula
         sozinha em v.concluiLabel. Importá-la direto inventaria data de
         conclusão real em tarefa que nem começou; então só entra quando
         alguém digitou uma data ali à mão. */
      var conclusaoReal = '';
      if(col.conclusao >= 0 && !cellHasFormula(ws, rb, col.conclusao)){
        conclusaoReal = serialToISO(cellNum(ws, rb, col.conclusao));
        if(conclusaoReal){
          avisos.push('"'+wbs+'": coluna Conclusão preenchida à mão ('+fmtDateBR(conclusaoReal)+') — importada como data de conclusão real.');
        }
      }
      ...
      alvo.tarefas.push({
        id: uid(),
        titulo: titulo,
        responsavel: cellText(ws, rb, col.resp),
        inicio: inicio,
        fim: fim,
        status: statusFinal,
    ...
    if(stats.canceladas){
      avisos.push(stats.canceladas+' tarefa(s) marcada(s) como cancelada/stand-by (CC) na planilha — importada(s) como "não iniciada".');
    }
    if(stats.marcas){
      avisos.push('As letras da linha "R" (C/D/A/R/N) entraram período a período: a linha R do Portal vai mostrar exatamente os períodos que estavam marcados na planilha, e não a duração planejada inteira. O status de cada tarefa é o da letra mais à direita.');
    }
    ...
    return {
      ...
      projeto: {
        id: uid(),
        nome: head.nome || ('Projeto importado — aba '+aba),
        lider: head.lider,
        unidade: unidade,
        dataRevisao: head.dataRevisao,
        /* horizonte fica vazio de propósito: computeHorizonKeys() já deriva
           das datas das tarefas, e os totais da planilha (linha ">> CRONOGRAMA
           DO PROJETO") são só MIN/MAX dessas mesmas datas. */
        inicio: '', fim: '', horizonteQtd: '',
        etapas: etapas,
      },
      avisos: avisos,
      stats: stats,
  ...
  window.crImportarXLSX = function(file){
    if(!file || !isEditable()) return;
    if(!/\.(xlsx|xlsm)$/i.test(file.name)){ toast('Escolha um arquivo .xlsx ou .xlsm.', true); return; }
    if(file.size > XLSX_MAX_BYTES){ toast('Planilha muito grande (máx. 15 MB).', true); return; }
    toast('Lendo a planilha…');
    loadSheetJS()
      .then(function(){ return readArrayBuffer(file); })
      .then(function(buf){
        // cellDates fica no default (desligado): serialToISO() quer o serial cru.
        var wb = window.XLSX.read(new Uint8Array(buf), { type:'array' });
        var results = [], ignoradas = [];
        (wb.SheetNames||[]).forEach(function(nm){
          var res = null;
          try{ res = parseEapSheet(wb.Sheets[nm], nm); }
          catch(err){ console.warn('[cronograma] erro ao ler a aba', nm, err); }
          if(res) results.push(res); else ignoradas.push(nm);
        });
        if(!results.length){
          toast('Nenhuma aba no formato EAP encontrada — procuro pela célula "ETAPAS / AÇÕES".', true);
          return;
        }
        abrirModalImportXLSX(results, ignoradas);
      })
      .catch(function(e){
        console.warn('[cronograma] falha ao importar planilha', e);
        toast('Não foi possível ler a planilha: '+((e && e.message) || 'erro desconhecido'), true);
      });
  };

  /* Nada é gravado antes desta conferência — é ela que substitui a necessidade
     de acertar toda heurística de leitura de primeira. */
  function abrirModalImportXLSX(results, ignoradas){
    var body = document.createElement('div');
    var html = '';
    results.forEach(function(res, i){
      var p = res.projeto, s = res.stats;
      var dup = DATA.projetos.some(function(x){ return x.nome === p.nome; });
      html += '<div class="cr-impcard">' +
        '<h4>'+esc(p.nome)+'</h4>' +
        '<div class="cr-impmeta">Aba <b>'+esc(res.aba)+'</b> · Líder <b>'+esc(p.lider||'—')+'</b>'+
          ' · Revisão <b>'+(p.dataRevisao?fmtDateBR(p.dataRevisao):'—')+'</b>'+
          ' · Unidade <b>'+esc(p.unidade)+'</b></div>' +
        '<div class="cr-impmeta"><b>'+p.etapas.length+'</b> etapa(s) · <b>'+s.total+'</b> tarefa(s): '+
          s.concluida+' concluída(s), '+s.em_andamento+' em andamento, '+s.nao_iniciada+' não iniciada(s)'+
          (s.marcas ? ' · <b>'+s.marcas+'</b> marca(s) de período na linha R' : '')+'</div>' +
        (dup ?
          '<div class="cr-impdup"><label>Já existe um projeto com esse nome:</label>' +
            '<select data-dup="'+i+'">' +
              '<option value="copia">Importar como cópia (mantém o atual)</option>' +
              '<option value="substituir">Substituir o projeto existente</option>' +
            '</select></div>'
          : '') +
        (res.avisos.length ?
          '<ul class="cr-impavisos">'+res.avisos.map(function(a){ return '<li>'+esc(a)+'</li>'; }).join('')+'</ul>'
          : '') +
        '</div>';
    });
    if(ignoradas.length){
      html += '<div class="cr-impmeta">Aba(s) ignorada(s) por não estarem no formato EAP: '+esc(ignoradas.join(', '))+'</div>';
    }
    body.innerHTML = html;
    openModal('Conferir antes de importar', body, [
      {label:'Cancelar', cls:'ghost', fn:closeModal},
      {label:'Importar', cls:'primary', fn:function(){ aplicarImportXLSX(results, body); }},
    ]);
  }
  function aplicarImportXLSX(results, body){
    if(!isEditable()) return;
    var ultimoId = null;
    results.forEach(function(res, i){
      var p = res.projeto;
      var sel = body.querySelector('[data-dup="'+i+'"]');
      if(sel && sel.value === 'substituir'){
        var idx = DATA.projetos.findIndex(function(x){ return x.nome === p.nome; });
        if(idx >= 0){
          p.id = DATA.projetos[idx].id; // preserva o id do projeto substituído
          DATA.projetos[idx] = p;
          ultimoId = p.id;
          return;
        }
    ...
    DATA = normalizeData(DATA); // mesma sanitização do import de JSON
    if(ultimoId) CUR_PROJ_ID = ultimoId;
    ensureCurrentProject();
    closeModal();
    markDirty(); render();
  ...
  /* Sobrou "Exportar projeto" como única exportação: é o caminho seguro pra
     ida-e-volta de dado, porque o arquivo de um projeto não carrega opinião
     sobre os outros. O "Exportar tudo" saiu em ago/2026 — ver o comentário do
     bloco EXPORT / IMPORT.

     Aqui só moram ações de EDIÇÃO, então a barra some inteira pra quem só lê /
     está em modo apresentação. Era o "Redefinir larguras" que obrigava a barra
     a renderizar sempre; ele foi pra toolbar do topo (é preferência de leitura,
     e a toolbar já reúne as outras — tela cheia, modo apresentação), o que
     também acabou com a duplicata que existia em crAlocTools(): a toolbar é
     única, então o botão vale pras duas visões sem repetir código. */
  ...
  /* Painel de leitura do FCA — o artefato que se lê em voz alta numa reunião de
     ...
     Renderiza nas DUAS visões, com e sem permissão de edição: em apresentação é
  ...
  /* Projeto "[PROPOSTA] X" é cópia de um projeto existente (replanejamento pra
     ...
     por importação de JSON/planilha no meio da sessão. Escolha explícita do
  ...
  function renderToolbarButtons(){
    ...
    // o toggle fica visível pra quem realmente pode editar, mesmo em modo
    // apresentação — senão não haveria como voltar ao modo edição.
    var btnPresent = document.getElementById('cr-btnpresent');
    if(btnPresent){
      btnPresent.style.display = (CAN_EDIT && !ocupacao) ? '' : 'none';
    ...
    // já a visão de ocupação é leitura pura: aparece pra qualquer um que veja
    // a página, inclusive quem não pode editar.
    ...
    // pra quem não pode editar — é leitura, não edição.
